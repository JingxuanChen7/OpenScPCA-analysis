...
run_anchorTrans <- function(ref_obj, sample,
  ...
  ggsave(plot = p, filename = paste0("plots/",sample,"_anchorTrans.pdf"), device = "pdf", width = 9, height = 6)
  p_split <- Seurat::DimPlot(sample_obj, reduction = "umap", group.by = "predicted.id", 
    ...
    ggtitle(sample)
  ggsave(plot = p_split, filename = paste0("plots/",sample,"_anchorTrans_split.pdf"), device = "pdf", width = 9, height = 9)
